...
function Body() {
  ...
  const categorySelected = async (id) => {
    ...
    const response = await fetch(
      "http://localhost:8080/productos/categoria/" + id
    );
    const data = await response.json();
  ...
  const filterProductsByDates = async (startDate, endDate) => {
    const response = await fetch(
      `http://localhost:8080/productos/${startDate}/${endDate}`
    );
    const data = await response.json();
    if (data.length == 0) {
      Swal.fire({
  ...
  const filterProductsByCityAndDates = async (city) => {
    ...
    const response = await fetch(
      `http://localhost:8080/productos/ciudad/${city.id}/${reservationDates.startDate}/${reservationDates.endDate}`
    );
    const data = await response.json();
    if (data.length == 0) {
      setIsFetchingProducts(false)
  ...
  const filterProductsByCity = async (city) => {
    setCitySelected(city.id);
    const response = await fetch(
      `http://localhost:8080/productos/ciudad/${city.id}`
    );
    const data = await response.json();
    if (data.length == 0) {
      setFilteredProducts([]);
